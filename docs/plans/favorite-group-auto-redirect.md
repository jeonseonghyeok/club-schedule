...
## 진행 상태
- [x] `group.html:71` "목록으로" 링크에 `list=1` 파라미터 추가
- [x] `MainController.main()` — 공통 꼬리 로직으로 리팩터 + 자동이동 조건 추가
- [x] 검증 방법 1~5 확인 — Playwright(하르온/9900000000004, 즐겨찾기=그룹2)로 확인:
  `/` 접근 시 `/groups/2`로 자동 리다이렉트 / "목록으로" 클릭 시 `/?list=1`로 이동해 자동
  리다이렉트 없이 홈 화면 유지 / 즐겨찾기 해제 후 `/` 접근 시 회귀 없이 홈 화면 그대로 /
  비로그인 상태 `/` 접근 시 기존과 동일하게 `/login/kakao/login_callback`으로 리다이렉트
